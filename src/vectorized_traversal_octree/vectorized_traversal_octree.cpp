#include "vectorized_traversal_octree.h"
#include <algorithm>
#include <limits>
#include <cmath>
...
VectorizedTraversalOctreeNode::VectorizedTraversalOctreeNode() 
    : min_x(0.0f), min_y(0.0f), min_z(0.0f), max_x(0.0f), max_y(0.0f), max_z(0.0f), is_leaf(true) 
{
    for (int i = 0; i < 8; ++i) {
        children[i] = nullptr;
        child_min_x[i] = std::numeric_limits<float>::infinity();
        child_max_x[i] = -std::numeric_limits<float>::infinity();
        child_min_y[i] = std::numeric_limits<float>::infinity();
        child_max_y[i] = -std::numeric_limits<float>::infinity();
        child_min_z[i] = std::numeric_limits<float>::infinity();
        child_max_z[i] = -std::numeric_limits<float>::infinity();
    }
}
...
VectorizedTraversalOctree::VectorizedTraversalOctree() : root_(nullptr) {}

VectorizedTraversalOctree::~VectorizedTraversalOctree() {
    if (root_) delete root_;
}

void VectorizedTraversalOctree::setInputCloud(const PointCloudSoA& cloud) {
    cloud_ = cloud;
}

void VectorizedTraversalOctree::build() {
    ...
    root_ = new VectorizedTraversalOctreeNode();
...
void VectorizedTraversalOctree::buildParams(VectorizedTraversalOctreeNode* node, const std::vector<int>& indices, int depth) {
    ...
    for (int i = 0; i < 8; ++i) {
        if (!child_indices[i].empty()) {
            node->children[i] = new VectorizedTraversalOctreeNode();
            ...
            // Store child bounds contiguously in parent
            node->child_min_x[i] = node->children[i]->min_x;
            node->child_max_x[i] = node->children[i]->max_x;
            node->child_min_y[i] = node->children[i]->min_y;
            node->child_max_y[i] = node->children[i]->max_y;
            node->child_min_z[i] = node->children[i]->min_z;
            node->child_max_z[i] = node->children[i]->max_z;

            buildParams(node->children[i], child_indices[i], depth + 1);
        } else {
            // Null child bounds already set to infinity in constructor
        }
    }
}

static inline bool boxOverlapsSphere(const VectorizedTraversalOctreeNode* node, const PointXYZ& center, float r2) {
...
std::size_t VectorizedTraversalOctree::radiusSearch(const PointXYZ& query, float radius,
    ...
    const VectorizedTraversalOctreeNode* stack[64];
    int stack_ptr = 0;
    stack[stack_ptr++] = root_;

    while (stack_ptr > 0) {
        const VectorizedTraversalOctreeNode* curr = stack[--stack_ptr];
        ...
        if (curr->is_leaf) {
            ...
            // Vectorized check of all 8 children
            size_t vl = __riscv_vsetvl_e32m2(8);

            vfloat32m2_t min_x = __riscv_vle32_v_f32m2(curr->child_min_x, vl);
            vfloat32m2_t max_x = __riscv_vle32_v_f32m2(curr->child_max_x, vl);
            vfloat32m2_t min_y = __riscv_vle32_v_f32m2(curr->child_min_y, vl);
            vfloat32m2_t max_y = __riscv_vle32_v_f32m2(curr->child_max_y, vl);
            vfloat32m2_t min_z = __riscv_vle32_v_f32m2(curr->child_min_z, vl);
            vfloat32m2_t max_z = __riscv_vle32_v_f32m2(curr->child_max_z, vl);

            vfloat32m2_t cx = __riscv_vfmin_vf_f32m2(max_x, query.x, vl);
            vfloat32m2_t cy = __riscv_vfmin_vf_f32m2(max_y, query.y, vl);
            vfloat32m2_t cz = __riscv_vfmin_vf_f32m2(max_z, query.z, vl);

            cx = __riscv_vfmax_vv_f32m2(min_x, cx, vl);
            cy = __riscv_vfmax_vv_f32m2(min_y, cy, vl);
            cz = __riscv_vfmax_vv_f32m2(min_z, cz, vl);

            vfloat32m2_t dx = __riscv_vfsub_vf_f32m2(cx, query.x, vl);
            vfloat32m2_t dy = __riscv_vfsub_vf_f32m2(cy, query.y, vl);
            vfloat32m2_t dz = __riscv_vfsub_vf_f32m2(cz, query.z, vl);
            ...
            vbool16_t mask = __riscv_vmfle_vf_f32m2_b16(d2, radius_sq, vl);

            uint8_t mask_val = 0;
            __riscv_vsm_v_b16(&mask_val, mask, vl);
...
std::size_t VectorizedTraversalOctree::radiusSearchScalar(const PointXYZ& query, float radius,
                                                       std::vector<int>& indices, std::vector<float>& dists) const {
    indices.clear();
    dists.clear();
    if (!root_) return 0;
