...
inline void savePCDCluster(const std::string& filename, const std::vector<PointXYZCluster>& points,
                           bool binary = false) {
    std::ofstream file(filename, binary ? (std::ios::binary | std::ios::out) : std::ios::out);
    if (!file.is_open()) {
        std::cerr << "Error: Could not open file " << filename << " for writing." << std::endl;
    ...
    file.close();
    std::cout << "Saved " << points.size() << " cluster points to " << filename << std::endl;
}
