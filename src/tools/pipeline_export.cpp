...
void saveStagePoints(const std::filesystem::path &path, const std::vector<PointXYZ> &points,
                      const char *label) {
...
int main(int argc, char **argv) {
  ...
  stage_start = std::chrono::high_resolution_clock::now();
  std::size_t n_down = voxel_grid_downsamp_rvv_v2(input_cloud, downsampled_pts.data(), voxel_leaf_size);
  ...
  if (skip_sor) {
    ...
    n_sor = sor_rvv(downsampled_cloud, sor_pts.data(), kPipelineConfig.sor_mean_k, kPipelineConfig.sor_std_threshold);
  ...
  stage_start = std::chrono::high_resolution_clock::now();
  int ransac_inliers_count = ransac_plane_rvv(sor_cloud, kPipelineConfig.ransac_distance_threshold,
