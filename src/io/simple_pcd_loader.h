...
inline int loadPCD(const std::string &file_path,
  ...
  for (std::size_t i = 0; i < header.fields.size(); ++i) {
    ...
    point_stride +=
        static_cast<std::size_t>(header.sizes[i] * header.counts[i]);
  }
  ...
  if (header.data_type == "binary") {
    ...
    for (std::size_t i = 0; i < header.points; ++i) {
      ...
      if (!detail::readFloat(
              point_base + point_offsets[static_cast<std::size_t>(x_index)],
              header.sizes[static_cast<std::size_t>(x_index)], x) ||
          !detail::readFloat(
              point_base + point_offsets[static_cast<std::size_t>(y_index)],
              header.sizes[static_cast<std::size_t>(y_index)], y) ||
          !detail::readFloat(
              point_base + point_offsets[static_cast<std::size_t>(z_index)],
              header.sizes[static_cast<std::size_t>(z_index)], z)) {
        std::cerr << "Error: Unsupported x/y/z field layout in binary PCD."
                  << std::endl;
        return -1;
      }
      points[i] = {x, y, z};
  ...
  if (header.data_type == "binary_compressed") {
    ...
    for (std::size_t i = 0; i < header.fields.size(); ++i) {
      ...
      cursor += static_cast<std::size_t>(header.sizes[i] * header.counts[i]) *
                header.points;
    }
    ...
    for (std::size_t i = 0; i < header.points; ++i) {
      ...
      const unsigned char *x_ptr =
          uncompressed.data() +
          field_block_offsets[static_cast<std::size_t>(x_index)] +
          i * static_cast<std::size_t>(
                  header.sizes[static_cast<std::size_t>(x_index)] *
                  header.counts[static_cast<std::size_t>(x_index)]);
      const unsigned char *y_ptr =
          uncompressed.data() +
          field_block_offsets[static_cast<std::size_t>(y_index)] +
          i * static_cast<std::size_t>(
                  header.sizes[static_cast<std::size_t>(y_index)] *
                  header.counts[static_cast<std::size_t>(y_index)]);
      const unsigned char *z_ptr =
          uncompressed.data() +
          field_block_offsets[static_cast<std::size_t>(z_index)] +
          i * static_cast<std::size_t>(
                  header.sizes[static_cast<std::size_t>(z_index)] *
                  header.counts[static_cast<std::size_t>(z_index)]);
      if (!detail::readFloat(
              x_ptr, header.sizes[static_cast<std::size_t>(x_index)], x) ||
          !detail::readFloat(
              y_ptr, header.sizes[static_cast<std::size_t>(y_index)], y) ||
          !detail::readFloat(
              z_ptr, header.sizes[static_cast<std::size_t>(z_index)], z)) {
    ...
    std::cout << "Loaded " << points.size() << " points (Binary Compressed)."
              << std::endl;
    return static_cast<int>(points.size());
  }

  std::cerr << "Error: Unsupported PCD DATA mode '" << header.data_type << "'."
            << std::endl;
  return -1;
}

inline void savePCD(const std::string &filename,
                    const std::vector<PointXYZ> &points, bool binary = false) {
...
inline void savePCDRGB(const std::string &filename,
  ...
  file.close();
  std::cout << "Saved " << points.size() << " colored points to " << filename
            << std::endl;
}
...
inline void savePCDCluster(const std::string &filename,
                           const std::vector<PointXYZCluster> &points,
                           bool binary = false) {
  std::ofstream file(filename, binary ? (std::ios::binary | std::ios::out)
                                      : std::ios::out);
  if (!file.is_open()) {
  ...
  if (binary) {
    for (const auto &p : points) {
      float cid_float = static_cast<float>(p.cluster_id);
      file.write(reinterpret_cast<const char *>(&p.x), sizeof(float));
      file.write(reinterpret_cast<const char *>(&p.y), sizeof(float));
      file.write(reinterpret_cast<const char *>(&p.z), sizeof(float));
      file.write(reinterpret_cast<const char *>(&cid_float), sizeof(float));
    }
  } else {
    for (const auto &p : points) {
      float cid_float = static_cast<float>(p.cluster_id);
      file << p.x << " " << p.y << " " << p.z << " " << cid_float << "\n";
    }
  }

  file.close();
  std::cout << "Saved " << points.size() << " cluster points to " << filename
            << std::endl;
}
