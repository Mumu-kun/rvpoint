  ...
  // no vsseg generated by the compiler.
  {
    ...
    while (i < in.n) {
      ...
      if (cnt > 0) {
        ...
        // Write compressed fields directly into AoS with stride=sizeof(PointXYZ)
        __riscv_vsse32_v_f32m8(base + count*3 + 0, stride,
            __riscv_vcompress_vm_f32m8(vx, mask, vl), (size_t)cnt);
        __riscv_vsse32_v_f32m8(base + count*3 + 1, stride,
            __riscv_vcompress_vm_f32m8(vy, mask, vl), (size_t)cnt);
        __riscv_vsse32_v_f32m8(base + count*3 + 2, stride,
            __riscv_vcompress_vm_f32m8(vz, mask, vl), (size_t)cnt);
        count += (size_t)cnt;
      }
      i += vl;
    }
