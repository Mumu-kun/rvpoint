...
std::size_t sor_caravan(const PointCloudSoA &in, PointXYZ *out, int k, float alpha) {
  ...
  while (q < in.n) {
    ...
    std::vector<std::vector<float>> tile_dists(vl);
    for (size_t lane = 0; lane < vl; ++lane) tile_dists[lane].reserve(in.n);
    ...
    for (size_t p = 0; p < in.n; ++p) {
      ...
      for (size_t lane = 0; lane < vl; ++lane) {
        tile_dists[lane].push_back(dist_buf[lane]);
      }
    }

    for (size_t lane = 0; lane < vl; ++lane) {
      std::partial_sort(tile_dists[lane].begin(), tile_dists[lane].begin() + k + 1, tile_dists[lane].end());
      float sum = 0.0f;
      for (int j = 1; j <= k; ++j) {
        sum += std::sqrt(tile_dists[lane][j]);
      }
      mean_dists[q + lane] = sum / k;
    }
