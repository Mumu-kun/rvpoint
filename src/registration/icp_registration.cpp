...
void correspondence_distance_rvv(const float* /*src_x*/, const float* /*src_y*/, const float* /*src_z*/,
    ...
    while (i < n_candidates) {
        ...
        // Store distances temporarily and find min
    ...
    for (std::size_t i = 0; i < n_candidates; ++i) {
        int idx = candidate_indices[i];
        float dx = tgt_x[idx] - qx;
        float dy = tgt_y[idx] - qy;
        float dz = tgt_z[idx] - qz;
...
void residual_jacobian_rvv(std::size_t n,
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        float tx = tgt_x[i], ty = tgt_y[i], tz = tgt_z[i];
        float nx = tgt_nx[i], ny = tgt_ny[i], nz = tgt_nz[i];

        float dx = sx - tx;
        float dy = sy - ty;
        float dz = sz - tz;

        float r = nx*dx + ny*dy + nz*dz;
        sum_error += std::abs(r);
...
void ICPRegistration::align(const PointCloudSoA& source_soa,
    ...
    SE3Transform current_transform;
    out_iterations = 0;
    out_error = 1e10f;
