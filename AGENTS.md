...
- **Leaf-Kernel Invariant & Dependency Injection**: Atomic compute kernels in `src/` must be pure leaf operators and must NEVER instantiate, embed, or own other compute kernels (e.g., `GroundFilter` must never own `RansacPlane`). All dependent kernels or scratch workspaces must be dependency-injected by reference (`Kernel&`) or decomposed into discrete DAG nodes orchestrated via `PipelineManager` (ADR-0012) to protect L1D cache working sets (ADR-0010) and enable multi-core thread scaling (ADR-0013).
