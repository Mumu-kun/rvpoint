...
"$PROJECT_ROOT/scripts/build.sh" --toolchain "$TOOLCHAIN" --backend "$BACKEND" --target "pipeline_export"
...
for pcd_path in "${PCD_FILES[@]}"; do
    ...
    "$PROJECT_ROOT/scripts/run.sh" --backend "$BACKEND" pipeline_export $SKIP_SOR $LEAF_SIZE "$pcd_path" "$frame_out_dir" > "${frame_out_dir}/pipeline.log" 2>&1
