...
// Fully hardened production version with uniform strided RANSAC, dynamic 3D
// spatial grid, self-cell fastpath ROR, and zero-copy SoA extraction.
...
inline const char *perceptionStatusToString(PerceptionStatus status) {
  switch (status) {
  case PerceptionStatus::SUCCESS:
    return "SUCCESS";
  case PerceptionStatus::NO_GROUND_PLANE_FOUND:
    return "NO_GROUND_PLANE_FOUND";
  case PerceptionStatus::INSUFFICIENT_INLIERS:
    return "INSUFFICIENT_INLIERS";
  case PerceptionStatus::DEGENERATE_POINT_CLOUD:
    return "DEGENERATE_POINT_CLOUD";
  case PerceptionStatus::SEARCH_INDEX_OVERFLOW:
    return "SEARCH_INDEX_OVERFLOW";
  case PerceptionStatus::IO_WRITE_FAILURE:
    return "IO_WRITE_FAILURE";
  case PerceptionStatus::INVALID_CLI_ARGUMENTS:
...
// Uses unified rvpoint::PipelineConfig from "core/pipeline_params.h"
...
struct FastPRNG {
  uint64_t state;
  explicit FastPRNG(uint64_t seed = 0x853c49e6748fea9bULL)
...
// True Radius Outlier Removal using Dynamic Spatial Hash Grid with Self-Cell
// Fastpath
static FusedResult execute_voxel_ror_rvv(const PointCloudSoA &cloud,
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    // 1. Fastpath: check self-cell first (skips 26 neighbor cells for dense
    // voxels)
    size_t self_h = grid.hash3D(qcx, qcy, qcz);
    int probe = 0;
    while (cells[self_h].head != -1 && probe < Fast3DSpatialGrid::kMaxProbes) {
      if (cells[self_h].cx == qcx && cells[self_h].cy == qcy &&
          cells[self_h].cz == qcz) {
    ...
    if (compute_normals || in_radius_count < min_neighbors) {
      for (int dz = -1;
           dz <= 1 && (compute_normals || in_radius_count < min_neighbors);
           ++dz) {
        for (int dy = -1;
             dy <= 1 && (compute_normals || in_radius_count < min_neighbors);
             ++dy) {
          for (int dx = -1;
               dx <= 1 && (compute_normals || in_radius_count < min_neighbors);
               ++dx) {
            if (dx == 0 && dy == 0 && dz == 0)
              continue;
            ...
            while (cells[h].head != -1 && p < Fast3DSpatialGrid::kMaxProbes) {
              if (cells[h].cx == tcx && cells[h].cy == tcy &&
                  cells[h].cz == tcz) {
                int curr = cells[h].head;
                while (curr != -1) {
                  float ddx = px[curr] - qx, ddy = py[curr] - qy,
                        ddz = pz[curr] - qz;
                  if (ddx * ddx + ddy * ddy + ddz * ddz <= r2) {
                    in_radius_count++;
                    if (compute_normals)
...
static FusedResult execute_fused_sor_normals_rvv(const PointCloudSoA &cloud,
                                                 const Fast3DSpatialGrid &grid,
  ...
  FusedResult res;
  if (valid_points.empty() || (n >= 20 && valid_points.size() < std::max<size_t>(10, n / 10))) {
    // If fewer than 10% of points had neighbors in sor_radius, the radius is severely
    // undersized for this cloud density. Preserve the downsampled cloud safely.
    res.x.assign(cloud.x, cloud.x + n);
    res.y.assign(cloud.y, cloud.y + n);
    res.z.assign(cloud.z, cloud.z + n);
    return res;
  }
  ...
  float dist_threshold =
      static_cast<float>(global_mean + (sor_alpha * global_std));
...
// ============================================================================
static bool compute_plane_coeffs(float x1, float y1, float z1, float x2,
                                 float y2, float z2, float x3, float y3,
                                 float z3, float *model) {
...
static int ransac_plane_sprt_rvv(const PointCloudSoA &cloud, float dist_thresh,
                                 int max_iters, float *model,
                                 const float *ground_normal_prior = nullptr,
                                 float min_ground_dot = 0.0f,
                                 uint64_t seed = 42) {
  if (cloud.n < 3)
    return 0;
  FastPRNG rng(seed);
  ...
  for (int iter = 0; iter < k_iters && iter < max_iters; ++iter) {
    ...
    float cand_model[4];
    if (!compute_plane_coeffs(
            cloud.x[i1], cloud.y[i1], cloud.z[i1], cloud.x[i2], cloud.y[i2],
            cloud.z[i2], cloud.x[i3], cloud.y[i3], cloud.z[i3], cand_model))
      continue;

    if (ground_normal_prior != nullptr && min_ground_dot > 0.0f) {
      float dot = std::abs(cand_model[0] * ground_normal_prior[0] +
                           cand_model[1] * ground_normal_prior[1] +
                           cand_model[2] * ground_normal_prior[2]);
      if (dot < min_ground_dot)
        continue;
    }

    float a = cand_model[0], b = cand_model[1], c = cand_model[2],
          d = cand_model[3];

    int inliers = 0;
#if defined(__riscv) || defined(__riscv_vector)
    size_t i = 0, n = cloud.n;
    while (i < n) {
      ...
      inliers += static_cast<int>(__riscv_vcpop_m_b4(mask_in, vl));
      i += vl;
    }
#else
    for (size_t pt = 0; pt < cloud.n; ++pt) {
      float dist =
          std::abs(a * cloud.x[pt] + b * cloud.y[pt] + c * cloud.z[pt] + d);
      if (dist <= dist_thresh)
        inliers++;
    }
#endif
...
static size_t extract_inliers_outliers_direct_soa(
    ...
    std::vector<PointXYZ> &inliers, std::vector<float> &ox,
...
#ifdef GEM5_BUILD
    alignas(64) float tx[kMaxVectorFloatsM8], ty[kMaxVectorFloatsM8],
        tz[kMaxVectorFloatsM8];
...
static std::vector<ClusterResult>
execute_union_find_clustering(const PointCloudSoA &cloud,
                              const Fast3DSpatialGrid &grid,
                              float tolerance, int min_cluster_size,
      ...
      for (int dy = -1; dy <= 1; ++dy) {
        for (int dx = -1; dx <= 1; ++dx) {
          ...
          while (grid.cells_[h].head != -1 &&
                 probe < Fast3DSpatialGrid::kMaxProbes) {
            if (grid.cells_[h].cx == tcx && grid.cells_[h].cy == tcy &&
                grid.cells_[h].cz == tcz) {
              int curr = grid.cells_[h].head;
              while (curr != -1) {
                if (static_cast<size_t>(curr) > i) {
                  float ddx = cloud.x[curr] - qx, ddy = cloud.y[curr] - qy,
                        ddz = cloud.z[curr] - qz;
...
void printFinalBreakdown(const std::vector<StageTiming> &stages,
  ...
  std::cout
      << "\n============================================================\n";
  std::cout << " [pipeline-ultimate] RVPoint RVV 1.0 Timing Breakdown\n";
  std::cout << "============================================================\n";
  for (const auto &st : stages) {
    const double pct = total_ms > 0.0 ? (st.ms / total_ms) * 100.0 : 0.0;
    std::cout << "[progress] [" << std::setw(2) << st.index << "/"
              << kStageCount << "] " << st.label << ": " << std::fixed
              << std::setprecision(3) << st.ms << " ms ("
              << std::setprecision(2) << pct << "%), pts=" << st.point_count
              << std::endl;
  }
  std::cout << "------------------------------------------------------------\n";
  std::cout << "  Total Pipeline Time (Compute Only): " << std::fixed
            << std::setprecision(3) << total_ms << " ms\n";
  std::cout << "  Pre-Pipeline File Load (Disk I/O) : " << std::fixed
            << std::setprecision(3) << io_load_ms << " ms\n";
  std::cout
      << "============================================================\n\n";
}
...
bool saveJSONMetrics(const std::filesystem::path &out_path,
                     ...
                     PerceptionStatus status, size_t inlier_count,
...
int main(int argc, char **argv) {
  ...
  bool skip_sor = false;
  bool use_ror = true; // Default to ROR matching PCL baseline
  bool skip_normals = false;
  bool disable_disk = false;
  PipelineConfig cfg = PipelineConfig::fromDelta(0.02f);
  float voxel_leaf_size = cfg.voxel_leaf_size;
  ...
  float min_ground_dot = 0.0f; // Unconstrained plane fitting by default (PCL equivalent)
  ...
  try {
    for (int i = 1; i < argc; ++i) {
      ...
      if (arg == "--progress") {
        ...
        cfg = PipelineConfig::fromPreset(argv[++i]);
        ...
        if (i + 1 >= argc) {
          std::cerr << "Error: Missing value for --ror-radius\n";
          return 1;
        }
        ror_radius = std::stof(argv[++i]);
        if (!std::isfinite(ror_radius) || ror_radius <= 0.0f) {
          std::cerr
              << "Error: --ror-radius must be a positive finite number.\n";
          return 1;
        }
      } else if (arg == "--ror-min-pts") {
        if (i + 1 >= argc) {
          std::cerr << "Error: Missing value for --ror-min-pts\n";
          return 1;
        }
        ...
        if (ror_min_pts < 1) {
          ...
          return 1;
                 ...
                 arg == "--no-normal" || arg == "--skip-normal") {
        skip_normals = true;
      } else if (arg == "--no-write" || arg == "--disable-disk") {
        disable_disk = true;
      } else if (arg == "--cluster-tolerance") {
        if (i + 1 >= argc) {
          std::cerr << "Error: Missing value for --cluster-tolerance\n";
          return 1;
        }
        cluster_tolerance = std::stof(argv[++i]);
        if (!std::isfinite(cluster_tolerance) || cluster_tolerance <= 0.0f) {
          std::cerr << "Error: --cluster-tolerance must be a positive finite "
                       "number.\n";
          return 1;
        }
      } else if (arg == "--min-cluster") {
        if (i + 1 >= argc) {
          std::cerr << "Error: Missing value for --min-cluster\n";
          return 1;
        }
        ...
        if (i + 1 >= argc) {
          std::cerr << "Error: Missing value for --max-cluster\n";
          return 1;
        }
        ...
        if (i + 1 >= argc) {
          std::cerr << "Error: Missing value for --ransac-iters\n";
          return 1;
        }
        ...
        if (i + 1 >= argc) {
          std::cerr << "Error: Missing value for --seed\n";
          return 1;
        }
        seed = std::stoull(argv[++i]);
      } else if (arg == "--ground-angle-thresh") {
        if (i + 1 >= argc) {
          std::cerr << "Error: Missing value for --ground-angle-thresh\n";
          return 1;
        }
        float deg = std::stof(argv[++i]);
        if (!std::isfinite(deg) || deg < 0.0f || deg > 90.0f) {
          std::cerr
              << "Error: --ground-angle-thresh must be in [0, 90] degrees.\n";
          return 1;
        }
  ...
  if (positional_args.empty()) {
    std::cerr
        << "Usage: " << argv[0]
        << " [--progress] [--json] [--no-write] [--no-normals] "
           "[--use-ror|--use-sor|--skip-sor] "
           ...
           "[--ground-angle-thresh <deg>] [--no-ground-prior] "
           "[--optical-frame] <input.pcd> [output_dir]\n";
    return 1;
  }

  if (min_cluster_size > max_cluster_size) {
    std::cerr << "Error: --min-cluster cannot be greater than --max-cluster."
              << std::endl;
    return 1;
  }
  ...
  const std::filesystem::path output_dir =
      positional_args.size() >= 2
          ...
          : std::filesystem::path("output") /
                (input_stem.string() + "_pipeline_ultimate");
  ...
  if (!disable_disk) {
    ...
    if (dir_ec) {
      std::cerr << "Error: Failed to create output directory '"
                << output_dir.string() << "': " << dir_ec.message()
                << std::endl;
      return 1;
    }
  }
  ...
  if (count <= 0 || loaded_points.empty()) {
    std::cerr << "Failed to load input PCD or cloud is empty: "
              << positional_args[0] << std::endl;
    return 1;
  }
  ...
  std::cout << "[pipeline-ultimate] [IO] Loaded Points: " << n_input
            << " in " << io_load_ms << " ms\n";
  ...
  // ── [1/5] Voxel Grid Downsampling (RVV) ───────────────────────────────────
  ...
  // ── [2/6] Build ROR SpatialGrid (explicit, isolated timing) ──────────────
  Fast3DSpatialGrid search_grid(
      use_ror ? ror_radius : cfg.sor_search_radius, n_down);
  const char *grid_label = use_ror ? "Build ROR SpatialGrid" : "Build SOR SpatialGrid";
  beginStage(2, grid_label, progress_enabled);
  stage_start = Clock::now();
  if (!skip_sor && n_down > 0) {
    search_grid.build(dx.data(), dy.data(), dz.data(), n_down);
  }
  stage_timings.push_back(
      {2, grid_label,
       endStage(2, grid_label, stage_start, progress_enabled),
       n_down});

  // ── [3/6] Outlier Removal (RVV, reuses pre-built spatial grid) ────────────
  const char *outlier_stage_label =
      skip_sor ? "Outlier removal (skipped)"
               : (use_ror ? "Radius Outlier Removal (ROR)"
                          : "Statistical Outlier Removal (SOR)");
  beginStage(3, outlier_stage_label, progress_enabled);
  stage_start = Clock::now();
  FusedResult fused;
  if (skip_sor) {
    fused.x = std::move(dx);
    ...
    fused = execute_voxel_ror_rvv(downsampled_cloud, search_grid, ror_radius,
                                  ror_min_pts, !skip_normals);
  } else {
    fused = execute_fused_sor_normals_rvv(
        downsampled_cloud, search_grid, cfg.sor_search_radius,
        cfg.sor_mean_k, cfg.sor_std_threshold,
        cfg.normal_k, !skip_normals);
  }
  size_t n_sor = fused.x.size();
  double fused_sor_ms =
      endStage(3, outlier_stage_label, stage_start, progress_enabled);
  stage_timings.push_back({3, outlier_stage_label, fused_sor_ms, n_sor});
  ...
  int r_cnt = 0;
  if (n_sor >= 3) {
    r_cnt = ransac_plane_sprt_rvv(
        sor_cloud, cfg.ransac_distance_threshold, ransac_max_iters,
        model, ground_normal_prior.data(), min_ground_dot, seed);
  }

  if (r_cnt < 50 ||
      (n_sor > 0 &&
       static_cast<float>(r_cnt) / static_cast<float>(n_sor) < 0.05f)) {
  ...
  size_t n_inliers = 0;
  if (r_cnt > 0) {
    n_inliers = extract_inliers_outliers_direct_soa(
        sor_cloud, model, cfg.ransac_distance_threshold, inlier_pts,
        ox, oy, oz, !disable_disk);
  } else {
    ox = fused.x;
    oy = fused.y;
    oz = fused.z;
  ...
  if (progress_enabled) {
    std::cout << "       [RANSAC] Model: (" << model[0] << ", " << model[1]
              << ", " << model[2] << ", " << model[3]
              << ") | Inliers: " << n_inliers
              << " (" << (n_sor > 0 ? (100.0f * n_inliers / n_sor) : 0.0f)
              << "%) | Non-Ground: " << n_outliers << "\n";
  }
  stage_timings.push_back(
      {4, "RANSAC Ground Plane",
       endStage(4, "RANSAC Ground Plane", stage_start, progress_enabled),
       n_outliers});

  // ── [5/6] Build Cluster SpatialGrid (explicit, isolated timing) ───────────
  beginStage(5, "Build Cluster SpatialGrid", progress_enabled);
  stage_start = Clock::now();
  Fast3DSpatialGrid cluster_grid(cluster_tolerance, n_outliers);
  if (n_outliers > 0) {
    cluster_grid.build(ox.data(), oy.data(), oz.data(), n_outliers);
  }
  stage_timings.push_back(
      {5, "Build Cluster SpatialGrid",
       endStage(5, "Build Cluster SpatialGrid", stage_start, progress_enabled),
       n_outliers});

  // ── [6/6] Euclidean Clustering (Flat Array Union-Find) ────────────────────
  ...
  auto clusters = execute_union_find_clustering(
      non_ground_cloud, cluster_grid, cluster_tolerance, min_cluster_size, max_cluster_size);
  stage_timings.push_back(
  ...
  // ── Post-Pipeline Disk Output (if enabled) ────────────────────────────────
  if (!disable_disk) {
    ...
    for (size_t c_idx = 0; c_idx < clusters.size(); ++c_idx) {
      ...
      for (int pt_idx : clusters[c_idx].indices) {
        if (pt_idx >= 0 && static_cast<size_t>(pt_idx) < n_outliers) {
          colored_pts.push_back(
              {ox[pt_idx], oy[pt_idx], oz[pt_idx], col.r, col.g, col.b});
        }
      }
    }
  ...
  if (json_metrics && !disable_disk) {
    if (!saveJSONMetrics(output_dir / "metrics.json", stage_timings, total_ms,
                         ...
                         pipeline_status, n_inliers, n_outliers,
                         clusters.size())) {
  ...
  if (pipeline_status == PerceptionStatus::IO_WRITE_FAILURE) {
    std::cerr
        << "Error: Output file write failure occurred during pipeline export."
        << std::endl;
    return 1;
  }
