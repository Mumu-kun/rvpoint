// ==============================================================================
// PIPELINE: eval/pipelines/pcl_octree_pipeline.cpp
// PURPOSE: Upstream PCL 1.14.0 — 5-Stage Perception Pipeline (Octree Backend).
//
//   Stage layout mirrors pipeline_3d_ultimate and pcl_native_pipeline:
//     [1/5] Voxel Grid Downsampling       pcl::VoxelGrid
//     [2/5] Radius Outlier Removal         pcl::RadiusOutlierRemoval (Octree rebuilt internally)
//     [3/5] RANSAC Ground Plane            pcl::SACSegmentation + pcl::ExtractIndices
//     [4/5] Build Cluster Octree           pcl::search::Octree  (explicit, timed separately)
//     [5/5] Euclidean Clustering           pcl::extractEuclideanClusters (pre-built tree, no rebuild)
//
//   Normal estimation: omitted.
//   Outlier removal: ROR (pcl::RadiusOutlierRemoval with Octree search backend).
//   Stage 2 Octree resolution = ror_radius. Stage 4 Octree resolution = cluster_tolerance.
// ==============================================================================
...
void printFinalBreakdown(const std::vector<StageTiming>& stages,
    ...
    std::cout << "\n============================================================\n";
    std::cout << " [pcl-octree] PCL 1.14 Octree Pipeline Timing Breakdown\n";
    std::cout << "============================================================\n";
...
int main(int argc, char** argv) {
    ...
    std::cout << "[pcl-octree] Loading: " << input_path
    ...
    if (!loadPCDToPCL(input_path, *cloud) || cloud->empty()) {
        std::cerr << "[pcl-octree] Error: Failed to load " << input_path << "\n";
        return 1;
    }
    const double io_load_ms =
        std::chrono::duration<double, std::milli>(Clock::now() - t_io_start).count();
    std::cout << "[pcl-octree] [IO] Loaded Points: " << cloud->size()
              << " in " << io_load_ms << " ms\n";
    ...
    // ── [2/6] Build ROR Octree (explicit, isolated timing) ──────────────────
    beginStage(2, "Build ROR Octree", progress_enabled);
    stage_start = Clock::now();
    pcl::search::Octree<pcl::PointXYZ>::Ptr ror_tree(
        new pcl::search::Octree<pcl::PointXYZ>(static_cast<double>(ror_radius)));
    if (!down_cloud->empty())
        ror_tree->setInputCloud(down_cloud);
    stage_timings.push_back({2, "Build ROR Octree",
        endStage(2, "Build ROR Octree", stage_start, progress_enabled),
        down_cloud->size()});

    // ── [3/6] Radius Outlier Removal (reuses pre-built octree) ───────────────
    ...
    // ── [5/6] Build Cluster Octree (explicit, isolated timing) ───────────────
    beginStage(5, "Build Cluster Octree", progress_enabled);
    stage_start = Clock::now();
    pcl::search::Octree<pcl::PointXYZ>::Ptr cluster_tree(
        new pcl::search::Octree<pcl::PointXYZ>(static_cast<double>(cluster_tolerance)));
    if (!non_ground->empty())
        cluster_tree->setInputCloud(non_ground);
    stage_timings.push_back({5, "Build Cluster Octree",
        endStage(5, "Build Cluster Octree", stage_start, progress_enabled),
        non_ground->size()});

    // ── [6/6] Euclidean Clustering (pre-built Octree, no rebuild) ────────────
