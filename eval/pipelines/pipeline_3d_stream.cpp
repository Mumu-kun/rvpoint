...
// Target Architecture: SpacemiT K1 / Orange Pi RV2 (RV64GCV Octa-Core) & QEMU /
// gem5
//
// Architectural Highlights:
// 1. Pre-Allocated Zero-Copy ThreadFrameContext per Core (No heap allocations
// in hot loop)
// 2. Lock-Free Asynchronous Frame Stream Worker Pool for Linear 8-Core
// Throughput Scaling
...
// 8. Verification cluster PCD export to output/stream_clusters/ in PointXYZRGB
// format
...
struct StreamConfig {
  ...
  bool progress = true;
  std::string mode =
      "inter"; // "inter" (Worker Pool) or "intra" (Single-Frame Low Latency)
...
public:
  ...
  void extract_centroids(std::vector<float> &dx, std::vector<float> &dy,
                         std::vector<float> &dz) {
    dx.clear();
    dy.clear();
    dz.clear();
...
struct alignas(64) ThreadFrameContext {
  ...
  ThreadFrameContext(float leaf_size = 0.10f, float ror_rad = 0.25f,
                     float clust_tol = 0.15f)
    ...
    fy.reserve(65536);
    fz.reserve(65536);
    sample_sx.reserve(2048);
    sample_sy.reserve(2048);
    sample_sz.reserve(2048);
    ox.reserve(65536);
    oy.reserve(65536);
    oz.reserve(65536);
...
static size_t execute_fast_radix_voxel_downsample_ctx(ThreadFrameContext &ctx,
                                                      size_t n,
                                                      float leaf_size) {
  if (n == 0)
    return 0;
  if (ctx.downsampled_pts.size() < n)
    ctx.downsampled_pts.resize(n);
  PointCloudSoA in{ctx.rx.data(), ctx.ry.data(), ctx.rz.data(), n};
  size_t n_down =
      voxel_grid_downsamp_rvv_v2(in, ctx.downsampled_pts.data(), leaf_size);
  ctx.dx.resize(n_down);
  ctx.dy.resize(n_down);
  ctx.dz.resize(n_down);

#if defined(RVV_PCL_USE_RVV) && defined(__riscv_vector)
  size_t i = 0;
  const float *src_ptr =
      reinterpret_cast<const float *>(ctx.downsampled_pts.data());
  while (i < n_down) {
    size_t vl = __riscv_vsetvl_e32m2(n_down - i);
    vfloat32m2x3_t seg = __riscv_vlseg3e32_v_f32m2x3(src_ptr + i * 3, vl);
    __riscv_vse32_v_f32m2(ctx.dx.data() + i,
                          __riscv_vget_v_f32m2x3_f32m2(seg, 0), vl);
    __riscv_vse32_v_f32m2(ctx.dy.data() + i,
                          __riscv_vget_v_f32m2x3_f32m2(seg, 1), vl);
    __riscv_vse32_v_f32m2(ctx.dz.data() + i,
                          __riscv_vget_v_f32m2x3_f32m2(seg, 2), vl);
    i += vl;
  }
#else
...
// ============================================================================
static size_t execute_ror_rvv_ctx(ThreadFrameContext &ctx, size_t n,
  ...
  if (ctx.ror_keep.size() < n)
    ctx.ror_keep.resize(n);
  uint8_t *keep = ctx.ror_keep.data();

#if defined(_OPENMP)
#pragma omp parallel for schedule(static, 64) if (!omp_in_parallel())
#endif
  for (size_t i = 0; i < n; ++i) {
...
static int ransac_plane_sprt_rvv_ctx(ThreadFrameContext &ctx, size_t n,
                                     float dist_thresh, int max_iters,
                                     float *model,
                                     const float *ground_normal_prior,
                                     float min_ground_dot, uint64_t seed = 42) {
  if (n < 3)
    return 0;
  FastPRNG rng(seed);
  ...
  for (int iter = 0; iter < k_iters && iter < max_iters; ++iter) {
    ...
    float cand_model[4];
    if (!compute_plane_coeffs(fx[i1], fy[i1], fz[i1], fx[i2], fy[i2], fz[i2],
                              fx[i3], fy[i3], fz[i3], cand_model))
      continue;
    ...
    while (si < sample_sz) {
      ...
      if (si == 256 && best_sample_inliers > 50) {
        int min_expected =
            (best_sample_inliers * 256) / static_cast<int>(sample_sz) / 3;
    ...
    if (sample_inliers > best_sample_inliers) {
      ...
      double w = static_cast<double>(best_sample_inliers) /
                 static_cast<double>(sample_sz);
  ...
  if (best_sample_inliers >= 10) {
    float a0 = best_model[0], b0 = best_model[1], c0 = best_model[2],
          d0 = best_model[3];
    ...
    if (inlier_cnt >= 10) {
      double cx = sum_x / inlier_cnt, cy = sum_y / inlier_cnt,
             cz = sum_z / inlier_cnt;
      ...
      for (int iter = 0; iter < 10; ++iter) {
        ...
        double s_norm = std::sqrt(shift_x * shift_x + shift_y * shift_y +
                                  shift_z * shift_z);
        if (s_norm > 1e-6) {
          vx = shift_x / s_norm;
          vy = shift_y / s_norm;
      ...
      if (ground_normal_prior != nullptr) {
        double dot = vx * ground_normal_prior[0] + vy * ground_normal_prior[1] +
                     vz * ground_normal_prior[2];
        if (dot < 0) {
          vx = -vx;
          vy = -vy;
          vz = -vz;
...
// ============================================================================
static size_t execute_clustering_ctx(ThreadFrameContext &ctx, size_t n,
  ...
  if (ctx.root_to_pts.size() < n)
    ctx.root_to_pts.resize(n);
  for (size_t i = 0; i < n; ++i)
    ctx.root_to_pts[i].clear();
  for (size_t i = 0; i < n; ++i) {
...
static bool export_clusters_pcd(const std::string &out_filepath,
                                const ThreadFrameContext &ctx) {
    ...
    for (int idx : ctx.clusters[c_idx].indices) {
      if (idx >= 0 && static_cast<size_t>(idx) < ctx.ox.size()) {
        colored_pts.push_back(
            {ctx.ox[idx], ctx.oy[idx], ctx.oz[idx], col.r, col.g, col.b});
      }
    }
  }
...
int main(int argc, char **argv) {
  ...
  if (pcd_files.empty()) {
    std::cerr << "Error: No .pcd files found in: " << cfg.input_path
              << std::endl;
    return 1;
  }

  if (cfg.max_frames > 0 &&
      pcd_files.size() > static_cast<size_t>(cfg.max_frames)) {
  ...
  std::cout
      ...
      << "  Target Architecture: SpacemiT K1 / Orange Pi RV2 (RV64GCV "
         "Octa-Core)\n"
      << "====================================================================="
         "===\n"
      << "  Input Directory : " << cfg.input_path << " (" << pcd_files.size()
      << " frames)\n"
      << "  Streaming Mode  : "
      << (cfg.mode == "inter"
              ? "INTER-FRAME (Asynchronous Multi-Core Stream Pool)"
              : ("INTRA-FRAME (Multi-Core Low Latency, " +
                 std::to_string(cfg.num_threads) + " Threads)"))
      << "\n"
      << "  Worker Threads  : " << cfg.num_threads << "\n"
      << "  Voxel Leaf Size : " << cfg.voxel_leaf_size << " m\n"
      << "  Cluster Tol     : " << cfg.cluster_tolerance
      << " m (min: " << cfg.min_cluster_size << ")\n"
      << "  Cluster Export  : "
      << (cfg.write_clusters ? "ENABLED (output/stream_clusters/)"
                             : "DISABLED (Zero Disk I/O)")
      << "\n"
      << "====================================================================="
         "===\n\n";
  ...
  for (int t = 0; t < pool_threads; ++t) {
    thread_contexts.emplace_back(cfg.voxel_leaf_size, cfg.ror_radius,
  ...
  auto process_single_frame = [&](size_t f_idx, int thread_id) {
    ...
    m.io_read_ms =
        std::chrono::duration<double, std::milli>(t_read_end - t_read_start)
            .count();

    if (load_res <= 0 || ctx.raw_pts.empty()) {
      std::cerr << "[Stream Warning] Frame " << f_idx
                << " failed to load: " << pcd_files[f_idx] << std::endl;
      return;
    }
...
#if defined(RVV_PCL_USE_RVV) && defined(__riscv_vector)
    size_t i = 0;
    const float *src_ptr = reinterpret_cast<const float *>(ctx.raw_pts.data());
    while (i < n_raw) {
      size_t vl = __riscv_vsetvl_e32m2(n_raw - i);
      vfloat32m2x3_t seg = __riscv_vlseg3e32_v_f32m2x3(src_ptr + i * 3, vl);
      __riscv_vse32_v_f32m2(ctx.rx.data() + i,
                            __riscv_vget_v_f32m2x3_f32m2(seg, 0), vl);
      __riscv_vse32_v_f32m2(ctx.ry.data() + i,
                            __riscv_vget_v_f32m2x3_f32m2(seg, 1), vl);
      __riscv_vse32_v_f32m2(ctx.rz.data() + i,
                            __riscv_vget_v_f32m2x3_f32m2(seg, 2), vl);
      i += vl;
    }
#else
    ...
    auto t1 = Clock::now();
    size_t n_down = execute_fast_radix_voxel_downsample_ctx(
        ctx, n_raw, cfg.voxel_leaf_size);
    auto t2 = Clock::now();
    ...
    auto t5 = Clock::now();
    size_t n_filtered =
        execute_ror_rvv_ctx(ctx, n_down, cfg.ror_radius, cfg.ror_min_neighbors);
    auto t6 = Clock::now();
    ...
    int inliers = ransac_plane_sprt_rvv_ctx(
        ctx, n_filtered, cfg.ransac_distance_threshold,
        cfg.ransac_max_iterations, plane_model, ground_normal_prior.data(),
        min_ground_dot, 42);
    auto t8 = Clock::now();
    ...
    auto t9 = Clock::now();
    size_t n_clusters =
        execute_clustering_ctx(ctx, ctx.ox.size(), cfg.cluster_tolerance,
    ...
    m.compute_ms =
        std::chrono::duration<double, std::milli>(t_comp_end - t_comp_start)
            .count();
    ...
    if (cfg.write_clusters) {
      ...
      std::stringstream ss;
      ss << "output/stream_clusters/frame_" << std::setfill('0') << std::setw(6)
         << f_idx << "_clusters.pcd";
      export_clusters_pcd(ss.str(), ctx);
      auto t_write_end = Clock::now();
      m.io_write_ms =
          std::chrono::duration<double, std::milli>(t_write_end - t_write_start)
              .count();
    }
...
#pragma omp critical
      {
        std::string tag =
            (cfg.mode == "inter")
                ? ("[Core " + std::to_string(thread_id) + "]")
                : ("[Intra " + std::to_string(cfg.num_threads) + "T]");
        std::cout << "[stream] " << std::setw(10) << std::left << tag
                  << " Frame " << std::setw(3) << f_idx << " (" << m.filename
                  << ") | "
                  << "Compute: " << std::fixed << std::setprecision(2)
                  << std::setw(6) << m.compute_ms << " ms ("
                  << std::setprecision(1) << std::setw(4) << fps << " FPS) | "
                  << "I/O Read: " << std::setprecision(2) << std::setw(5)
                  << m.io_read_ms << " ms | "
                  << "Pts: " << std::setw(6) << m.raw_points << " -> "
                  << std::setw(5) << m.downsampled_points << " | "
                  << "Obstacles: " << std::setw(5) << m.obstacle_points << " | "
                  << "Clusters: " << std::setw(2) << m.cluster_count
                  << std::endl;
      }
    }
  };
...
#pragma omp parallel for schedule(dynamic, 1)
  ...
  double total_wall_ms = std::chrono::duration<double, std::milli>(
                             stream_end_wall - stream_start_wall)
                             .count();
  ...
  double stream_wall_fps =
      (total_wall_ms > 0.0) ? (count * 1000.0 / total_wall_ms) : 0.0;

  std::cout << "\n============================================================="
               "===========\n"
            << "  CONTINUOUS STREAM PERCEPTION BENCHMARK SUMMARY (" << count
            << " frames)\n"
            << "==============================================================="
               "=========\n"
            << "  [Overall Stream Throughput & Wall-Clock Runtime]:\n"
            << "    • Total Wall-Clock Time   : " << std::fixed
            << std::setprecision(2) << total_wall_ms << " ms\n"
            << "    • Sustained Stream Rate   : " << std::setprecision(2)
            << stream_wall_fps << " FPS ("
            << (cfg.mode == "inter" ? "Asynchronous Worker Pool"
                                    : "Single Stream")
            << ")\n"
            << "---------------------------------------------------------------"
               "---------\n"
            << "  [Pure Compute Pipeline Latency] (Excluding all Disk I/O):\n"
            << "    • Average Compute Latency : " << std::fixed
            << std::setprecision(2) << avg_comp << " ms\n"
            << "    • Min Compute Latency     : " << min_comp << " ms\n"
            << "    • Max Compute Latency     : " << max_comp << " ms\n"
            << "    • Per-Frame Compute Rate  : " << std::setprecision(2)
            << effective_fps << " FPS\n"
            << "---------------------------------------------------------------"
               "---------\n"
            << "  [Compute Stage Breakdown (Average per Frame)]:\n"
            << "    1. Voxel Downsample       : " << std::setw(6)
            << (sum_vox / count) << " ms (" << std::setprecision(1)
            << ((sum_vox / sum_comp) * 100.0) << "%)\n"
            << "    2. Fast Spatial Grid Build: " << std::setw(6)
            << (sum_grid / count) << " ms (" << std::setprecision(1)
            << ((sum_grid / sum_comp) * 100.0) << "%)\n"
            << "    3. RVV Radius Outlier Rem : " << std::setw(6)
            << (sum_ror / count) << " ms (" << std::setprecision(1)
            << ((sum_ror / sum_comp) * 100.0) << "%)\n"
            << "    4. RVV RANSAC Ground Fit  : " << std::setw(6)
            << (sum_ransac / count) << " ms (" << std::setprecision(1)
            << ((sum_ransac / sum_comp) * 100.0) << "%)\n"
            << "    5. Spatial Clustering     : " << std::setw(6)
            << (sum_clust / count) << " ms (" << std::setprecision(1)
            << ((sum_clust / sum_comp) * 100.0) << "%)\n"
            << "---------------------------------------------------------------"
               "---------\n"
