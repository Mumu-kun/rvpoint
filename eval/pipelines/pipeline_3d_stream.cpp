    ...
    ThreadFrameContext& ctx, size_t n, float leaf_size)
{
    ...
    size_t i = 0;
    const float* src_ptr = reinterpret_cast<const float*>(ctx.downsampled_pts.data());
    while (i < n_down) {
        size_t vl = __riscv_vsetvl_e32m2(n_down - i);
        vfloat32m2x3_t seg = __riscv_vlseg3e32_v_f32m2x3(src_ptr + i * 3, vl);
        __riscv_vse32_v_f32m2(ctx.dx.data() + i, __riscv_vget_v_f32m2x3_f32m2(seg, 0), vl);
        __riscv_vse32_v_f32m2(ctx.dy.data() + i, __riscv_vget_v_f32m2x3_f32m2(seg, 1), vl);
        __riscv_vse32_v_f32m2(ctx.dz.data() + i, __riscv_vget_v_f32m2x3_f32m2(seg, 2), vl);
        i += vl;
    }
#else
...
int main(int argc, char** argv) {
    ...
    auto process_single_frame = [&](size_t f_idx, int thread_id) {
        ...
        while (i < n_raw) {
            ...
            vfloat32m2x3_t seg = __riscv_vlseg3e32_v_f32m2x3(src_ptr + i * 3, vl);
            __riscv_vse32_v_f32m2(ctx.rx.data() + i, __riscv_vget_v_f32m2x3_f32m2(seg, 0), vl);
            __riscv_vse32_v_f32m2(ctx.ry.data() + i, __riscv_vget_v_f32m2x3_f32m2(seg, 1), vl);
            __riscv_vse32_v_f32m2(ctx.rz.data() + i, __riscv_vget_v_f32m2x3_f32m2(seg, 2), vl);
            i += vl;
        }
#else
