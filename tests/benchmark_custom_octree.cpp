#include "custom_rvv_octree/custom_pointer_octree.h"
...
int main(int argc, char** argv) {
    ...
    std::printf("============================================================\n");
    std::printf("  RVPoint Custom-Intrinsic Octree Benchmark (Fallback Mode)\n");
    std::printf("============================================================\n\n");
    ...
    for (int leaf_size : leaf_sizes) {
        std::printf("------------------------------------------------------------\n");
        std::printf("  Testing Custom Octree (Leaf Size: %d)\n", leaf_size);
        std::printf("------------------------------------------------------------\n");

        CustomPointerOctree octree;
        ...
        std::printf("  Scalar Instructions: %lu | Custom RVV (Fallback) Instructions: %lu | Speedup: %.2fx\n",
                    sc_instructions, rvv_instructions, inst_gain);
        std::printf("  Scalar Time: %.2f ms | Custom RVV (Fallback) Time: %.2f ms | Speedup: %.2fx\n",
