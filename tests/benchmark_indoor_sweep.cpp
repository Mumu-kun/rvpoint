...
int main(int argc, char** argv) {
    ...
    std::printf("============================================================\n");
    std::printf("  RVPoint Indoor Point Cloud Pipeline Hyperparameter Sweep\n");
    std::printf("============================================================\n\n");
    ...
    std::printf("Loaded raw point cloud with %d points.\n", n);

    // Output file configuration
    std::ofstream report("/workspace/results/indoor_sweep_report.txt");
    if (!report.is_open()) {
    ...
    print_line("# Indoor Point Cloud Hyperparameter Sweep Report\n\n");
    ...
    const float voxel_sizes[] = {0.005f, 0.01f, 0.02f, 0.03f, 0.05f};
    const int leaf_capacities[] = {8, 16, 24, 32, 48, 64};
    const float search_radii[] = {0.01f, 0.02f, 0.03f, 0.05f, 0.08f};
    ...
    report.close();
    std::printf("\n[SUCCESS] Hyperparameter sweep completed. Report written to: /workspace/results/indoor_sweep_report.txt\n");
    return 0;
}
