...
int main() {
    ...
    // Note: We can't easily count iterations inside the lib without modifying it, but VLENB proves the HW state.
    size_t count_rvv = voxel_grid_downsamp_rvv(input_soa, out_rvv.data(), LEAF);
    
    // Print VLEN for confirmation (read CSR)
    size_t vlenb;
    asm volatile("csrr %0, vlenb" : "=r"(vlenb));
    std::cout << "DEBUG: VLENB = " << vlenb << " bytes (" << (vlenb*8) << " bits)" << std::endl;
    // LMUL=8 -> 8 registers group. Elements/reg = vlenb / 4 bytes. 
    std::cout << "DEBUG: Effective Vector Width (m8) = " << (vlenb/4)*8 << " floats per instruction" << std::endl;
